...
<td style="padding: 0.25rem 0.5rem; font-size: 8.5pt; text-align: left;">
...
<td style="padding: 0.25rem 0.5rem; font-size: 10pt; color: #4B5563; font-weight: 600;">${
      product.scale ?? ""
    }</td>
<td style="text-align: right; padding: 0.25rem; padding-right: 20px; font-size: 8.5pt;">
  <div style="display: flex; flex-direction: column; align-items: end; justify-content: flex-start; height: 100%;">
    ...
    <span style="font-size: ${product.discount ? "9pt" : "11pt"}; ${
      product.discount ? "text-decoration: line-through;" : "font-weight: bold;"
...
<td style="font-size: 11pt; text-align: right; padding: 0.25rem; color: #4B5563; font-weight: 600;">${
      product.srp
    }</td>
...
function renderProductCards(products) {
  ...
  products.forEach((product) => {
    const card = `
<div style="max-width: 100%; background-color: white; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1); border-radius: 8px; overflow: hidden; border: 1px solid #E5E7EB; padding: 12px; display: flex; flex-direction: column; align-items: start;">
  <div style="display: flex; gap: 8px; justify-content: space-between; width: 100%;">
    <div style="flex: 0 0 40%; display: flex; flex-direction: column; gap: 4px;">
      <!-- Image (smaller, and aligned) -->
      <img
        style="width: 100%; height: 96px; object-fit: contain; border-radius: 4px;"
        src="${product.image}"
        alt="${product.name}"
      />
      <div style="font-size: 8pt; color: #6B7280;">Scale: <span style="font-weight: bold;">${product.scale}</span></div>
    </div>
    <div style="font-size: 8pt; flex: 1; color: #333333; display: flex; flex-direction: column; gap: 4px;">
      <h2 style="font-size: 8pt; font-weight: bold; line-height: 1.2;">${product.name}</h2>
      <p style="font-size: 8.5pt; font-weight: 600;">${product.description}</p>
      <div style="font-size: 8pt; color: #16A34A; font-weight: bold; display: flex; align-items: center; gap: 4px;">
        <i class="fas fa-check-circle"></i>
        <div>In Stock</div>
      </div>
      <p style="color: #374151;">SKU: <span style="font-weight: 600;">${product.sku}</span></p>
      <p style="color: #374151;">MFG Code: <span style="font-weight: 600;">${product.mfgCode}</span></p>
      <p style="color: #374151;">UPC: <span style="font-weight: 600;">${product.upc}</span></p>
      <p style="color: #374151;">Category: <span style="font-weight: bold;">${product.category}</span></p>
    </div>
  </div>
  <div style="display: flex; justify-content: space-between; width: 100%;">
    <div style="display: flex; flex-direction: column; align-items: start;">
      <div style="font-size: 8pt; color: #16A34A; font-weight: 600;">Reserve on ${product.reserve}</div>
      <div style="font-size: 8pt; color: #CA8A04; font-weight: 600;">ETA: ${product.eta}</div>
    </div>
    <div style="display: flex; align-items: center; gap: 8px; margin-top: 8px;">
      <input type="number" style="width: 48px; padding: 4px; font-size: 12px; border: 1px solid #D1D5DB; border-radius: 4px;" placeholder="2">
      <button style="background-color: #F59E0B; color: white; font-size: 12px; font-weight: 600; padding: 4px 12px; border-radius: 4px;">Add to Cart</button>
    </div>
  </div>
</div>
`;
